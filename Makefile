...
DBG_TEST_TARGET := 		flvtestd.exe

REL_DYNAMIC_TARGET :=	libflv.so
REL_STATIC_TARGET := 	libflv.a
REL_TEST_TARGET := 		flvtest.exe 
...
%.do:%.cpp
	$(CXX) -c -o $@ $(DBG_CXXFLAGS) $(LIBS) $<
	
%.o:%.cpp
	$(CXX) -c -o $@ $(REL_CXXFLAGS) $(LIBS) $<
	
clean:
	rm -rf $(DBG_OBJS) $(DBG_TEST_TARGET) $(DBG_STATIC_TARGET) $(DBG_DYNAMIC_TARGET) $(REL_OBJS) $(REL_TEST_TARGET) $(REL_STATIC_TARGET) $(REL_DYNAMIC_TARGET)
